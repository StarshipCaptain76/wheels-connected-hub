...
/** Escape user text for safe HTML email bodies. */
...
export const sendContactMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const html =
      ...
      '<tr><td style="padding:6px 0;color:#666">From</td><td style="padding:6px 0"><strong>' +
      esc(data.name) +
      "</strong></td></tr>" +
      '<tr><td style="padding:6px 0;color:#666">Email</td><td style="padding:6px 0"><a href="mailto:' +
      esc(data.email) +
      '">' +
      esc(data.email) +
      "</a></td></tr>" +
      '<tr><td style="padding:6px 0;color:#666">Mobile</td><td style="padding:6px 0"><a href="tel:' +
      esc(data.phone.replace(/\s/g, "")) +
      '">' +
      esc(data.phone) +
      "</a></td></tr>" +
      '<tr><td style="padding:12px 0 6px;color:#666;vertical-align:top">Message</td>' +
      '<td style="padding:12px 0 6px;white-space:pre-wrap">' +
      esc(data.message) +
      "</td></tr>" +
      "</tbody></table>" +
